...
export const LibraryFlowV2: React.FC<LibraryFlowV2Props> = ({ onNavigateToReading, onNavigateToStatistics, onToggleNav }) => {
  ...
  // --- Custom Fast Scroll State & Touch Event Handlers for Mobile + Desktop ---
  ...
  const scrollerThumbHeight = 56;

  // Perform content scroll calculation based on client height
  const handleScrollerPointerDown = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const targetThumbTop = clickY - scrollerThumbHeight / 2;
    const trackHeight = trackRect.height;
    const maxThumbTop = trackHeight - scrollerThumbHeight;
    const boundedThumbTop = Math.max(0, Math.min(targetThumbTop, maxThumbTop));
    
    const ratio = maxThumbTop > 0 ? boundedThumbTop / maxThumbTop : 0;
    const totalScrollable = document.documentElement.scrollHeight - window.innerHeight;
    const targetScrollY = ratio * totalScrollable;
    
    window.scrollTo({ top: targetScrollY, behavior: 'instant' as any });
    
    setIsDraggingScroller(true);

    const handleGlobalPointerMove = (moveEvent: PointerEvent) => {
      if (!trackRef.current) return;
      moveEvent.preventDefault();

      const innerRect = trackRef.current.getBoundingClientRect();
      const moveClickY = moveEvent.clientY - innerRect.top;
      const moveTargetThumbTop = moveClickY - scrollerThumbHeight / 2;
      const moveTrackHeight = innerRect.height;
      const moveMaxThumbTop = moveTrackHeight - scrollerThumbHeight;
      const moveBoundedThumbTop = Math.max(0, Math.min(moveTargetThumbTop, moveMaxThumbTop));
      
      const moveRatio = moveMaxThumbTop > 0 ? moveBoundedThumbTop / moveMaxThumbTop : 0;
      const moveTotalScrollable = document.documentElement.scrollHeight - window.innerHeight;
      const moveTargetScrollY = moveRatio * moveTotalScrollable;
      
      window.scrollTo({ top: moveTargetScrollY, behavior: 'instant' as any });
    };

    const handleGlobalPointerUp = () => {
      setIsDraggingScroller(false);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      fadeTimeout = setTimeout(() => {
        if (isMounted) setIsScrolling(false);
      }, 1500);
    };
  ...
  const trackHeightCalculated = scrollerState.clientHeight > 0 ? scrollerState.clientHeight - 96 - 112 : 300;
  const maxThumbTopCalculated = Math.max(0, trackHeightCalculated - scrollerThumbHeight);
  
  const scrollerRatio = scrollerState.scrollHeight - scrollerState.clientHeight > 0 
    ? scrollerState.scrollY / (scrollerState.scrollHeight - scrollerState.clientHeight)
    : 0;
    
  const scrollerThumbTop = scrollerRatio * maxThumbTopCalculated;
  const showScroller = route.kind === 'list' && (scrollerState.scrollHeight - scrollerState.clientHeight > 100);
  ...
  return (
    <div className="px-4 pb-24 text-gray-800 space-y-4">
      ...
      {/* Custom Fast-Scroll Interactive Scrollbar for Desktop/Mobile dragging */}
      {showScroller && (
        <div
          ref={trackRef}
          onPointerDown={handleScrollerPointerDown}
          className={`fixed right-1 top-24 bottom-28 w-8 z-40 flex items-center justify-center cursor-pointer select-none transition-opacity duration-300 ${
            isScrolling || isDraggingScroller ? 'opacity-100' : 'opacity-0 pointer-events-none'
          }`}
          style={{ touchAction: 'none' }}
        >
          {/* Inner visible handle of width and thumb height */}
          <div className="h-full w-4 flex justify-center items-start relative select-none">
            {/* Scroll Track Line */}
            <div className="absolute top-0 bottom-0 w-0.5 bg-gray-200/40 rounded-full" />
            {/* Scroll Thumb/Runner */}
            <div
              className={`absolute w-2.5 rounded-full select-none transition-all duration-75 ${
                isDraggingScroller ? 'bg-indigo-600 scale-x-125 shadow-lg shadow-indigo-200' : 'bg-gray-400/70 hover:bg-slate-500'
              }`}
              style={{
                height: `${scrollerThumbHeight}px`,
                transform: `translateY(${scrollerThumbTop}px)`,
                touchAction: 'none',
              }}
            />
          </div>
